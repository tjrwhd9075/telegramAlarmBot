...
def get_name(bot, update):
    global korea
    global usa
    if bot.channel_post.text != None:
        msg = bot.channel_post.text.upper()               #  최근 입력된 메시지의 텍스트
    ...
    if korea == 1:
        ...
        if codefind(msg,"krx") != 0 : # 종목이 목록에 있으면
            button_list = build_button(["30일","100일","300일","cancel"], msg)
            show_markup = InlineKeyboardMarkup(build_menu(button_list, len(button_list) - 1))
            update.bot.edit_message_text(text=msg+" 기간을 선택해 주세요.",
                                        chat_id=chat_id,
...
def get_command(bot, update):
    ...
    show_list3 = []
    show_list3.append(InlineKeyboardButton("binance", callback_data="binance3")) # add on button
    show_list3.append(InlineKeyboardButton("upbit", callback_data="upbit3")) # add off button
    show_list3.append(InlineKeyboardButton("korea", callback_data="korea3")) # add off button
    show_list3.append(InlineKeyboardButton("usa", callback_data="usa3")) # add off button
    show_list3.append(InlineKeyboardButton("cancel", callback_data="cancel")) # add cancel button
    show_markup3 = InlineKeyboardMarkup(build_menu(show_list3, len(show_list3) - 1)) # make markup

    if msg == "/s":
        bot.effective_message.reply_text("Heiken Ashi chart", reply_markup=show_markup)
    elif msg == "/m":
        bot.effective_message.reply_text("signal maker", reply_markup=show_markup3)
    elif msg == "/t":
    ...
    if len(data_selected.split(",")) == 1 :
        ...
        #ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
            ...
            update.bot.edit_message_text(text="봉을 선택해 주세요.",
                                        ...
                                        message_id=bot.callback_query.message.message_id,
                                        reply_markup=show_markup)
        elif data_selected == "korea2": 
            update.bot.edit_message_text(text="종목명을 입력해주세요(대소문자, 띄어쓰기 주의)",
                                        chat_id=bot.callback_query.message.chat_id,
                                        message_id=bot.callback_query.message.message_id)
            korea = 2
        elif data_selected == "usa2": 
            update.bot.edit_message_text(text="티커를 입력해주세요(대소문자 주의)",
                                        chat_id=bot.callback_query.message.chat_id,
                                        message_id=bot.callback_query.message.message_id)
            # global usa ; usa = 1 # 전역변수
        ...
        #ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
            ...
            update.bot.edit_message_text(text="봉을 선택해 주세요.",
                                        ...
                                        message_id=bot.callback_query.message.message_id,
                                        reply_markup=show_markup)
        elif data_selected == "korea3": 
            update.bot.edit_message_text(text="종목명을 입력해주세요(대소문자, 띄어쓰기 주의)",
                                        chat_id=bot.callback_query.message.chat_id,
                                        message_id=bot.callback_query.message.message_id)
            korea = 3
        elif data_selected == "usa3": 
            update.bot.edit_message_text(text="티커를 입력해주세요(대소문자 주의)",
                                        chat_id=bot.callback_query.message.chat_id,
                                        message_id=bot.callback_query.message.message_id)
            # global usa ; usa = 1 # 전역변수
        ...
        #ㅡㅡHA 조회ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
        if  name == "binance" :  # 바이낸스 검색
            ...
            update.bot.edit_message_text(text=bot.callback_query.data + " KRW-BTC의 HA 차트입니다",
                                        chat_id=bot.callback_query.message.chat_id,
                                        message_id=bot.callback_query.message.message_id)
        #ㅡㅡ백테스트ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
            ...
            for cnt in count :
                ...
                update.bot.sendMessage(text= data_selected.split(",")[0] + " "+ data_selected.split(",")[-1] +" "+ coin +"\n\n원금 1,000,000원\n" + text,
                                            chat_id=bot.callback_query.message.chat_id)
        # ㅡㅡㅡ시그널메이커ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
        elif  name == "binance3" :   # 바이낸스 시그널메이커
            if data_selected.split(",")[-1] == "1d": interval = '1d'
            elif data_selected.split(",")[-1] == "4h": interval = '4h'
            elif data_selected.split(",")[-1] == "1h": interval = '1h'
            elif data_selected.split(",")[-1] == "30m": interval = '30m'
            ...
            update.bot.sendMessage(text="💲💲 "+data_selected.split(",")[0] + " "+ coin +" " + data_selected.split(",")[-1] +" 💲💲\n" +\
        ...
        #ㅡㅡㅡHA 조회ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
        if korea == 1:
            korea = 0
            count = 0
            if data_selected.split(",")[-1] == "30일":
                count = 30
            elif data_selected.split(",")[-1] == "100일":
                count = 100
            elif data_selected.split(",")[-1] == "300일":
                count = 300
            
            # 한국종목 HA 계산, 차트 띄워
            plot_candle_chart(heiken_ashi_jusik(name, "krx", count),name)
            telbot.send_photo(chat_id=bot.callback_query.message.chat_id, photo=open(image, 'rb'))  # 사진보내기
            time.sleep(1)
...
# 매일 정해진 시간에
schedule.every().day.at("17:30").do(lambda:us_ha_check()) 
schedule.every().day.at("23:00").do(lambda:us_ha_check())
